...
module ScoutApm
  class StackProfile
    ...
    def self.gc_event_datas_for(start_time, end_time)
      event_datas = []
      gc_event_datas.select{|evnt| (evnt[:end_gc_count] > 0) and times_overlap?(evnt[:start_time], evnt[:end_time], start_time, end_time) }.each do |gc_data_hash|
    ...
    def self.times_overlap?(gc_start_time, gc_end_time, start_time, end_time)
      ...
      return true if (start_time > gc_start_time) and (end_time < gc_end_time)
    ...
    def rss_increased?
      @gc_events.inject(0){|total,evnt| total += evnt.rss_size_diff; total} > 0
    end
  end
end
